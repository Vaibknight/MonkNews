...
export class News extends Component {
  ...
  fetchMoreData = async () => {
    ...
    const url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apikey=7365ee0e4a3f4f2b9ebbfbbc4e207288&page=${this.state.page}&pageSize=${this.props.pageSize}`;
    let data = await fetch(url);
    let parsedData = await data.json()
    this.setState({
    ...
    return (
      <div>
          <h2 className="text-center " style={{ margin: "40px 0px" }}>
            Vaibnews - Top {this.toCapitalCase(this.props.category)} Headlines{" "}
          </h2>
